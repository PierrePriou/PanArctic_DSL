...
map_vectors <- velo_new_HD %>%
  filter(between(x, -40, 40) & between(y, -41, 40)) %>%
  ggplot() +
  # Plot interpolated current velocity
  geom_contour_fill(aes(x = x, y = y, z = velocity), binwidth = 0.5, na.fill = T) +
  # Coast
  ...
  # Current vectors
  geom_arrow(aes(x = x, y = y, dx = vxo, dy = vyo, mag = velocity), skip = 4, size = 0.2, 
...
```{r new-projection-raster-HD}
# Set new projection 
res_HD <- c(25, 25) # in km
r_HD <- raster(extent(extent_current), crs = proj_current, resolution = res_HD / 100)

# Convert to spatial data
velo_HD <- SpatialPointsDataFrame(
  SpatialPoints(cbind(velo$x, velo$y), proj4string = crs(r_current)),
  data.frame(lat = velo$lat,
             lon = velo$lon,
             vxo = velo$vxo,
             ...
             thetao = velo$thetao)) %>%
  # Rasterize on the new grid
  rasterize(., r_HD, fun = mean, na.rm = T) %>% 
  # Remove ID layer
  dropLayer(1) %>% 
  # Convert raster to data frame
  rasterToPoints() %>% 
  ...
  # Add year
  mutate(velocity = sqrt(vxo^2 + vyo^2)) %>%
  complete(x = seq(min(x - (res_HD[1] / 100)), max(x + (res_HD[1] / 100)), res_HD[1] / 100), 
           y = seq(min(y - (res_HD[2] / 100)), max(y + (res_HD[2] / 100)), res_HD[2] / 100)) %>%
