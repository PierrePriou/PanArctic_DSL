...
map_velo <- velo_new %>%
  ...
  geom_tile(data = subset(velo_new, is.na(vxo) == T), aes(x = x, y = y), fill = "white", col = "white") +
  # Coast
  geom_polygon(data = coast, aes(x = x, y = y, group = group), fill = "grey30") +
  # Current vectors
  ...
  # Scales
  scale_fill_viridis_c("V", option = "viridis", limits = c(0, 5), end = 1, 
                       oob = scales::squish, alpha = 0.5, breaks = seq(0, 10, 1)) +
...
map_base <- velo_new %>%
  ...
  geom_point(aes(x = 0, y = 0), shape = 4, size = 0.7, stroke = 0.3, col = "white") +
  # GAM locations
  # geom_tile(data = SA_new, aes(x = x, y = y), fill = NA, colour = "red", size = 4) +
  # Scales
  # scale_fill_viridis_c("", option = "plasma", limits = c(0, 5), end = 1, 
  #                      oob = scales::squish, alpha = 0.5, breaks = seq(0, 10, 1)) +
  scale_fill_cmocean("", name = "speed", limits = c(0, 5), end = 0.9, 
                       oob = scales::squish, alpha = 0.5, breaks = seq(0, 10, 1)) +
...
map_vectors <- velo_new %>%
  ...
  # Current vectors
  geom_arrow(aes(x = x, y = y, dx = vxo, dy = vyo, mag = velocity), size = 0.2, arrow.angle = 30,
             arrow.type = "open", arrow.length = 0.75) +
  # Arctic circle
  # geom_path(data = arctic_circle, aes(x = x, y = y), col = "white", lty = 2, size = 0.3) +
  # GAM locations
  # geom_tile(data = SA_new, aes(x = x, y = y), fill = NA, colour = "red", size = 3.8) + 
  # Scales
  scale_fill_viridis_c("", option = "viridis", limits = c(0, 5), end = 1,
                       oob = scales::squish, alpha = 0, breaks = seq(0, 10, 1)) +
  coord_fixed(xlim = c(-40, 40), ylim = c(-40, 40), expand = F) +
  guides(mag = "none", alpha = "none") +
  theme(legend.position = "right",
        legend.key.height = unit(0.3, "in"),
        ...
        axis.ticks = element_blank(),
        panel.border = element_rect(fill = NA, size = 0.3))
ggsave("plots/map_vectors.png", map_vectors, bg = "transparent",
