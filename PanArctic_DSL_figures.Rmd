...
LME <- readOGR("data/arctic_regions/LME/LME_2013_polygon.shp",
  ...
  mutate(LME_study = if_else(id %in% c("4", "11", "14", "15", "16"), "a", "b"))
...
proj_bathy_laea <- raster(extent(-3500, 3500, -3500, 3500), 
...
longitudes <- data.frame(lon = rep(seq(-180, 180, 45), each = 200)) %>%
  group_by(lon) %>%
  mutate(lat =  seq(0, 80, length.out = 200)) %>%
  ungroup() %>%
  # Transform into sf
  st_as_sf(coords = c("lon", "lat"), crs = st_crs("EPSG:4326"), remove = F) %>% 
  # Change projection
  st_transform(crs = st_crs(arctic_laea)) %>%
  mutate(xc = st_coordinates(.)[,1],
         yc = st_coordinates(.)[,2], 
         lon = factor(lon)) %>%
  st_drop_geometry()
...
map_laea <- bathy_laea %>%
  ...
  # LME
  geom_polygon(data = subset(LME, LME_study == "a"), 
               aes(x = xc, y = yc, group = group, col = LME_study), 
               col = "#F05223", fill = NA, lwd = 0.35, lty = 1) +
  # Plot coastlines
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group),
               fill = "grey30") +
  # Graticules 
  geom_path(data = latitudes, aes(x = xc, y = yc, group = lat), col = "grey95", size = 0.4, lty = 2) +
  geom_path(data = longitudes, aes(x = xc, y = yc, group = lon), col = "grey95", size = 0.4, lty = 2) +
  geom_path(data = can_usa, aes(x = xc, y = yc, group = lon), col = "grey5", size = 0.4) +
  # Arctic circle
  ...
  geom_point(data = MVBS_laea, aes(x = xc, y = yc), 
             shape = 20, col = met.brewer("Johnson", n = 5)[4],#"#009E73",
             alpha = 0.5) +
  # Trawls
  geom_point(data = trawl_laea, aes(x = xc, y = yc),
             shape = 23, fill = "#E69F00", size = 2) +
  # Plot North Pole
  geom_point(aes(x = 0, y = 0), shape = 4, size = 1, stroke = 0.7, col = "#F05223") +
...
ggsave("plots/base_map_laea_stations.png", map_laea, height = 4, width = 6, dpi = 600, units = "in")
...
map_RA <- trawl_station_df %>%
  ...
  geom_scatterpie(data = trawl_station_df, aes(x = xc_jit,  y = yc_jit),
                  cols = c("Arctic cod", 
                           ...
                           "Redfish",
                           "Other"),
                  alpha = 0.9, pie_scale = 1.75, legend_name = "Species", 
...
GAMSAI_velo <- predSA_I %>%
  ...
  scale_fill_manual(name = "LME",
                    values = met.brewer("Johnson", n = 6, direction = -1)) +
  coord_cartesian(xlim = c(-0.05, 4.05), ylim = c(-4, 32), expand = F) +
  ...
  theme(panel.border = element_blank(),
        ...
        plot.margin = unit(c(0, 0, 0, 0), "in"))
# Save and display plot
ggsave("plots/Figure_HGAM_SAI3.png", GAMSAI_velo, height = 2.5, width = 6.5, dpi = 600, units = "in")
GAMSAI_velo
```
...
map_mycto <- data.frame(
  ...
  new_scale_fill() +
  # Plot 200 m isobath
  # geom_contour(data = bathy_laea, aes(z = depth), breaks = -200, col = "black", lwd = 0.1, alpha = 0.8) +
  # Plot LME
  # geom_polygon(data = LME, aes(x = xc, y = yc, group = group),
  #              col = "black", fill = NA, alpha = 0.2,
  #              lwd = 0.35, lty = 1) +
  # Plot coastlines
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group), alpha = 0,
               fill = "grey30") +
  # geom_path(data = can_usa, aes(x = xc, y = yc, group = lon), col = "grey5", size = 0.2) +
  # Arctic circle
  # geom_path(data = arctic_circle_laea, aes(x = xc, y = yc), 
  #           col = "white", lty = 2, size = 0.4) +
  ...
  geom_point(aes(x = xc, y = yc, fill = var), shape = 21,
             size = 2, alpha = 0.8) +
  # Plot North Pole
  # geom_point(aes(x = 0, y = 0), shape = 4, size = 1, stroke = 0.7) +
  coord_fixed(xlim = c(-2700, 1650), ylim = c(-2400, 2300), expand = F) +
...
map_mycto_nopoint <- data.frame(
  ...
  theme(legend.position = "right", 
        ...
        axis.title = element_blank())
# Save and display plots
ggsave("plots/map_myctophid_nopoints.png", map_mycto_nopoint,
       height = 4, width = 6, dpi = 600, units = "in")
# map_mycto

map_mycto_base <- data.frame(
  lon = c(133 + (32.27/60), # AWI 1995
          133 + (34.9/60), # AWI 1995
          -179.99, # Zhang 2022
          25.5, # Snoeijs Leijonmalm et al. 2022
  ...
  ggplot(aes(x = xc, y = yc)) +
  # Plot bathy
  geom_raster(data = subset(bathy_laea, depth >= -200), aes(fill = depth_d), alpha = 1) +
  scale_fill_manual("Depth (m)", values = c("#E8E8E8", "#DBDBDB", "red")) +
  guides(fill = "none") +
  new_scale_fill() +
  # Plot coastlines
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group),
  ...
  theme(legend.position = "right", 
        ...
        axis.title = element_blank())
# Save and display plots
ggsave("plots/map_myctophid_base.png", map_mycto_base,  bg = 'transparent',
       height = 4, width = 6, dpi = 600, units = "in")
# map_mycto_base
```
...
# ICES/PICES presentation plots

## Maps

```{r ICES-map-currents, fig.height=4, fig.width=6}
...
map_laea <- bathy_laea %>%
  ...
  geom_raster(aes(fill = depth_d), alpha = 0.8) +
  scale_fill_grey("Depth (m)", start = 0.9, end = 0.3) +
  # scale_fill_cmocean("Depth (m)", name = "deep", limits = c(-4500, 0),
  #                    direction = -1, breaks = seq(-6000, 0, 500),
  #                    na.value = "#281A2CCC") +
  # guides(fill = "none") + 
  new_scale_fill() +
  # LME
  ...
  theme(legend.position = "right", 
        ...
        axis.ticks = element_blank(), 
        axis.title = element_blank())
ggsave("plots/ICES_base_map_laea.png",
       map_laea, height = 6.5, width = 10, dpi = 600, units = "in")
...
map_laea <- bathy_laea %>%
  ...
  theme(legend.position = "right", 
        ...
        axis.ticks = element_blank(), 
        axis.title = element_blank())
ggsave("plots/ICES_base_map_laea_bathy_only.png",
       map_laea, height = 6.5, width = 10, dpi = 600, units = "in")
...
map_laea <- bathy_laea %>%
  ...
  theme(legend.position = "right", 
        ...
        axis.ticks = element_blank(), 
        axis.title = element_blank())
ggsave("plots/ICES_base_map_laea_land_only.png", bg = 'transparent',
       map_laea, height = 6.5, width = 10, dpi = 600, units = "in")
...
boxplot_NASC <- SA_grid_laea %>%
  ...
  mutate(mean_NASC = mean(NASC_int)) %>%
  ggplot(aes(x = LME)) +
  geom_boxplot(aes(y = NASC_int, col = LME),
               # size = 0.4, outlier.size = 0.7,
               alpha = 0.8, position = position_dodge(preserve = "single")) +
  # geom_point(aes(y = 10*log10(mean_NASC), fill = LME), shape = 23, size = 2.5) +
  # scale_x_discrete(labels = c("BF", "BB", "BA")) +
  scale_y_continuous(trans = "log10",
    expression("Integratedc s"[A]*" (m"^2*" nmi"^-2*")"),
    position = "left",
    breaks = c(0.1, 1, 10, 100, 1000, 10000),
    labels = scales::comma)+
  scale_colour_manual(values =  c("#132B69", "#03739B", "#93AA43")) +
  scale_fill_manual(values =  c("#132B69", "#03739B", "#93AA43")) +
  # coord_cartesian(ylim = c(-4.5, 44.5)) +
  theme(legend.position = "none",
...
lm_SA_lat <- LM_area_res %>%
  ggplot() + 
  geom_point(data = subset(SA_lm, LME != "Nares Strait"),
             aes(x = lat, y = SA_int, col = LME, shape = shape),
             alpha = 0.5, size = 3) +
  geom_line(aes(x = lat, y = .fitted, col = LME), size = 1) + 
  geom_ribbon(aes(x = lat, ymin = .lower, ymax = .upper,
                  group = LME, fill = LME),
              alpha = 0.15) +
  scale_x_continuous("Latitude (°N)", breaks = seq(0, 90, 1)) + 
  scale_y_continuous(expression("S"[A]*" (dB re 1 m"^2*" nmi"^-2*")")) +
  # scale_shape_manual("LME", values = c(17, 16)) +
  scale_colour_manual("LME", values = col_pal) +
  scale_fill_manual("LME", values = col_pal) +
  coord_cartesian(ylim = c(-4.5, 44.5)) +
  guides(fill = "none", shape = "none") +
...
# Combine plots
p_boxplot_NASC_LM <- plot_grid(boxplot_NASC, lm_SA_lat, rel_widths = c(0.4, 0.7),
                               ncol = 2, align = "hv", axis = "tblr")
# Save and display plot
ggsave("plots/ICES_boxplot_SA_LM.png", p_boxplot_NASC_LM,
       height = 4.5, width = 13, dpi = 600, units = "in")
# Save and display plot
ggsave("plots/ICES_boxplot_NASC.png", boxplot_NASC,
       height = 5, width = 6, dpi = 600, units = "in")
# Save and display plot
ggsave("plots/ICES_LM_NASC_latitude.png", lm_SA_lat,
       height = 4.5, width = 6, dpi = 600, units = "in")
```
...
map_velo <- phy_grid_laea_hd %>%
  ...
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group), 
               fill = "grey25") + 
  coord_fixed(xlim = c(-2450, 1200), ylim = c(-2000, 1950), expand = F) +
  # Plot North Pole
  geom_point(aes(x = 0, y = 0), shape = 4, stroke = 0.5) +
  theme(legend.title = element_text(hjust = 0.5),
        legend.key.width = unit(0.1, "in"),
...
# Save and display plot
ggsave("plots/ICES_map_velo.png", map_velo, 
       height = 2.5, width = 2.5, dpi = 600, units = "in")
# map_velo
```
## Map NASC
...
map_DSL <- SA_grid_laea %>%
  ...
  theme(legend.position = "right",
        ...
        axis.title = element_blank())
# Save and display plot
ggsave("plots/ICES_map_NASC.png", map_DSL, 
       height = 6, width = 6.5, dpi = 600, units = "in")
```
...
ICES_GAMSAI_velo <- predSA_I %>%
  ...
  scale_fill_manual(name = "LME",
                    values = met.brewer("Johnson", n = 6, direction = -1)) +
  coord_cartesian(xlim = c(-0.05, 4.6), ylim = c(-2, 32), expand = F) +
  scale_x_continuous(breaks = seq(0, 10, 0.5)) +
  # scale_y_continuous(expression("S"[A]*" (dB re 1 m"^2*" nmi"^-2*")")) +
  scale_linetype_manual(name = "LME", values = c(2, 2, 1)) +
  labs(x = expression("Current velocity at 318 m (cm s"^-1*")"),
       y = expression("S"[A]*" (dB re 1 m"^2*" nmi"^-2*")")) +
  theme(panel.border = element_blank(),
        axis.line = element_line(),
        legend.title = element_blank(),
        ...
        plot.margin = unit(c(0, 0, 0, 0), "in"))
# Save and display plot
ggsave("plots/ICES_HGAM_SAI3.png", ICES_GAMSAI_velo, 
       height = 3.5, width = 8, dpi = 600, units = "in")
ICES_GAMSAI_velo
```
# OLD PLOTS
## LM map DSL
