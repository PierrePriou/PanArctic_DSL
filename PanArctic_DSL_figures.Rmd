...
boxplot_NASC <- SA_grid_laea %>%
  mutate(LME = factor(case_when(LME == "BF" ~ "Beaufort Sea",
    ...
    year = factor(year)) %>%
  ggplot(aes(x = LME)) +
  geom_boxplot(aes(y = 10*log10(NASC_int), col = year),
  ...
  # scale_colour_manual(values =  c("#132B69", "black", "#03739B",
  #                                 "#93AA43", "grey50")) +
  scale_colour_manual(values =  c("#132B69", "#03739B", 
                                  "#93AA43", "grey50"),
...
map_RA <- trawl_station_df %>%
  ...
  coord_fixed(xlim = c(-2450, 650), ylim = c(-1500, 1850), expand = F) +
  geom_point(aes(x = 0, y = 0), shape = 23, stroke = 0.2, col = "white",
             fill = "#C26744") +  # Legend over 2 rows
...
GAMSAI_velo <- predSA_I %>%
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(trans = ~10^(./10), 
                        name = expression("s"[A]*" ( m"^2*" nmi"^-2*")"),
                        breaks = c(1, 5, 10, 50, 100, 500, 1000, 5000, 10000))) +
  scale_linetype_manual(name = "LME", values = c(1, 1, 1)) +
...
map_mycto <- data.frame(
  lon = c(133 + (32.27/60), # AWI 1995
          ...
          -179.99, # Zhang 2022
          25.5, # Snoeijs Leijonmalm et al. 2021
          -122.357, # Frosti 2021
          -117.90, # Frosti 2021
          -117.088, # Frosti 2021
  ...
  lat = c(78 + (06.02/60), # AWI 1995
          ...
          75.04, # Zhang 2022
          88.0, # Snoeijs Leijonmalm et al. 2021
          70.896, # Frosti 2021
          70.342, # Frosti 2021
          70.332, # Frosti 2021
  ...
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group),
               fill = "grey30") +
  geom_point(aes(x = xc, y = yc, fill = var), shape = 21, size = 3, alpha = 0.8) + 
...
```{r WMD-boxplot}
WMD_all_year <- SA_grid_laea %>%
  mutate(year = 9999)
boxplot_WMD <- bind_rows(SA_grid_laea, NASC_all_year) %>%
  mutate(LME = factor(case_when(LME == "BF" ~ "Beaufort Sea",
                                LME == "NAR" ~ "Nares Strait",
                                LME == "BB" ~ "Baffin Bay",
                                LME == "SV" ~ "Northern\nBarents Sea",
    ...
    year = factor(year, levels = c(2015, 2016, 2017, 9999))) %>%
  ggplot(aes(x = LME)) +
  geom_boxplot(aes(y = CM, col = year),
               size = 0.4, outlier.size = 0.7,
               alpha = 0.8, position = position_dodge(preserve = "single")) +
  scale_y_reverse() +
  scale_colour_manual(values =  c("#132B69", "#03739B", 
...
map_WMD <- SA_grid_laea %>%
  ...
  scale_fill_manual("WMD (m)", 
                    limits = c("(250,300]", "(300,350]", 
                               ...
                               "(550,600]"),
                    values = col_pal,
                    labels = c("251-300", "301-350", "351-400", 
                                  "401-450", "451-500", "501-550", 
                                  "551-600")) +
  geom_point(aes(x = 0, y = 0), shape = 23, stroke = 0.2, col = "white",
             fill = "#C26744") +
