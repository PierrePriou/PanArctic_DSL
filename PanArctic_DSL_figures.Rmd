...
GAMSAI_velo <- predSA_I %>%
  filter(var == "v") %>%
  ggplot() +
  geom_line(aes(x = v, y = SA_fit, col = IHO), size = 0.6) +
...
# GAM chlorophyll
GAMSAI_chl <- predSA_I %>%
  mutate(IHO = as.character(IHO),
         IHO = factor(if_else(IHO == "WAO_BF", "WAO & BF", IHO),
                      levels = c("WAO & BF", "CAA", "BB", "DS", "EAO"))) %>%
  filter(var == "chl") %>%
  ggplot() +
  geom_line(aes(x = chl, y = SA_fit, col = IHO), size = 0.6) +
...
HGAM <- plot_grid(legend, 
                  plot_grid(GAMSAI_velo, GAMSAI_chl, ncol = 2, labels = "auto", label_size = 9),
