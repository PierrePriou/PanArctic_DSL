...
```{r summary-region}
# Calculate percent of DSL in all grid cells across all areas
DSL_all_area <- SA_grid_laea %>%
  group_by(empty_clean, year) %>% # per year
  summarise(n = n()) %>%
  ungroup() %>%
  group_by(year) %>%
  mutate(total = sum(n)) %>%
  ...
  group_by(empty_clean) %>%
  summarise(percent = mean(percent)) %>% # mean percentage across all years
  filter(empty_clean == F) %>%
  mutate(IHO_area = "All areas") %>%
  dplyr::select(IHO_area, percent)

# Calculate percent of DSL within each area
DSL_area <- SA_grid_laea %>% 
  group_by(IHO_area, empty_clean, year) %>% # per year
  summarise(n = n()) %>%
  ungroup() %>%
  group_by(IHO_area, year) %>% 
  mutate(total = sum(n)) %>%
  ungroup() %>%
  group_by(IHO_area) %>% # per area per year
  mutate(percent = round(n / total, 2) * 100) %>%
  ungroup() %>%
  group_by(IHO_area, empty_clean) %>% # per area
  summarise(percent = round(mean(percent))) %>% # mean percentage across all years
  filter(empty_clean == F) %>%
  dplyr::select(IHO_area, percent) %>%
  bind_rows(., DSL_all_area)

# Calculate median SA across all areas
SA_all_area <- SA_grid_laea %>%
  filter(empty_clean == F) %>%
  group_by(year) %>% # per year
  summarise(n = n(),
            SA_int_median = round(median(10 * log10(NASC_int_clean)), 1),
            SA_int_mad = round(mad(10 * log10(NASC_int_clean)), 1),
            NASC_median = round(median(NASC_int_clean), 1),
            NASC_mad = round(mad(NASC_int_clean), 1),
            CM_median = round(median(CM)),
            CM_mad = round(mad(CM))) %>%
  ungroup() %>%
  summarise(n = sum(n), # Mean across all areas and years
            SA_int_median = round(mean(SA_int_median), 1),
            SA_int_mad = round(mean(SA_int_mad), 1),
            NASC_median = round(mean(NASC_median), 1),
            NASC_mad = round(mean(NASC_mad), 1),
            CM_median = round(mean(CM_median)),
            CM_mad = round(mean(CM_mad))) %>%
  mutate(IHO_area = "All areas")
...
SA_grid_laea %>%
  ...
  left_join(., DSL_area, by = "IHO_area") %>%
  mutate(IHO_area = factor(case_when(IHO_area == "East Arctic Ocean" ~ "East Arctic\nOcean",
...
## Base map with areas reprojcted
...
p_Sv_prof_lines <- Sv_prof_median %>%
  ...
  scale_colour_manual(values = met.brewer("Hiroshige", n = 6, direction = -1)) +
  scale_fill_manual(values = met.brewer("Hiroshige", n = 6, direction = -1)) +
...
map_DSL <- SA_grid_laea %>%  # Map of DSL detections
  mutate(DSL = factor(if_else(empty_clean == T, F, T), levels = c(T, F))) %>%
  arrange(DSL) %>%
  ggplot(aes(x = xc,  y = yc)) +
  geom_raster(data = bathy_laea, aes(x = xc, y = yc, fill = depth_d)) + # bathy
  scale_fill_cmocean("Depth (m)", name = "deep", discrete = T, na.value = NA, alpha = 0.4) +
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group), fill = "grey80") + # Coast
  geom_point(aes(col = DSL), size = 0.6) +
  scale_color_manual("DSL presence", values = c("#1170aa", "#fc7d0b")) +
  facet_wrap(~ year) +
  coord_fixed(xlim = c(-2450, 600), ylim = c(-1500, 1800), expand = F) +
  # coord_fixed(xlim = c(-2600, 1100), ylim = c(-1600, 1900), expand = F) +
  guides(fill = "none",
         colour = guide_legend(override.aes = list(size = 2))) + # Increase symbol size in legend
  theme(legend.position = "top",
        axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank())

col_pal <- c("#1170aa", "#fc7d0b", "#76B7B2") # Custom colour palette
...
# p_boxplot_SA <- 
  ...
  mutate(IHO_area = factor(case_when(IHO_area == "East Arctic Ocean" ~ "East Arctic\nOcean",
                           ...
                           levels = c("West Arctic\nOcean", "Beaufort Sea", "Can. Arctic\nArchipelago",
                                      "Baffin Bay", "Davis Strait", "East Arctic\nOcean"))) %>%
  filter(empty_clean == F) %>%
  ggplot(aes(x = IHO_area, y = SA_int_clean)) + 
  geom_boxplot(position = position_dodge(preserve = "single")) +
  scale_y_continuous(expression("S"[A]*" (dB re 1 m"^2*" nmi"^-2*")")) +
  scale_fill_manual(values = col_pal) +
  theme(axis.title.x = element_blank(), 
        legend.title = element_blank(),
        legend.position = c(0.075, 0.78),
        plot.margin = unit(c(0.1, 0.1, 0, 0), "in"))

p_map_SA_DSL <- plot_grid(map_DSL, p_boxplot_SA, ncol = 1, align = "v", axis = "lr") 
ggsave("plots/Fig3_map_boxplot_SA.png", p_map_SA_DSL, height = 5, width = 6, dpi = 600, units = "in")
p_map_SA_DSL
```

```{r}
test <- SA_grid_laea %>% # Boxplot of SA per region
  mutate(IHO_area = factor(case_when(IHO_area == "East Arctic Ocean" ~ "East Arctic\nOcean",
                                     IHO_area == "West Arctic Ocean" ~ "West Arctic\nOcean",
                                     IHO_area == "Beaufort Sea" ~ "Beaufort Sea",
                                     IHO_area == "The Northwestern Passages" ~ "Can. Arctic\nArchipelago",
  ...
  filter(empty_clean == F) %>%
  group_by(IHO_area, year) %>%  # median per year
  summarise(NASC_median = median(NASC_int_clean),
            NASC_05 = quantile(NASC_int_clean, 0.05),
            NASC_25 = quantile(NASC_int_clean, 0.25),
            NASC_75 = quantile(NASC_int_clean, 0.75),
            NASC_95 = quantile(NASC_int_clean, 0.95),
            NASC_mad = mad(NASC_int_clean)) %>%
  ungroup() %>%
  group_by(IHO_area) %>% # Mean per area
  summarise(NASC_median = mean(NASC_median),
            NASC_05 = mean(NASC_05),
            NASC_25 = mean(NASC_25),
            NASC_75 = mean(NASC_75),
            NASC_95 = mean(NASC_95),
            NASC_mad = mean(NASC_mad)) %>%
  ungroup() #%>% 
  
test %>%
  ggplot(aes(x = IHO_area, y = NASC_median, col = IHO_area)) + 
  geom_pointrange(aes(ymin = NASC_median - NASC_mad, ymax = NASC_median + NASC_mad))

  geom_crossbar(aes(ymin = NASC_25, ymax = NASC_75)) +
  geom_pointrange(aes(ymin = NASC_05, ymax = NASC_95))
