...
p_map_laea <- bathy_laea %>%
  ...
  geom_path(data = arctic_circle_laea, aes(x = xc, y = yc), 
            col = "white", lty = 2, size = 0.5) +
  coord_fixed(xlim = c(-2450, 650), ylim = c(-1500, 1800), expand = F) +
  guides(fill = "none") +
  # Acoustics
  geom_point(data = SA_raw_laea, aes(x = xc, y = yc), 
             shape = 4, col = "darkblue", alpha = 0.5) +
  # Trawls
...
ggsave("plots/base_map_laeamap_stations.png",
       p_map_laea, height = 4, width = 6, dpi = 600, units = "in")
p_map_laea
```
...
lm_SA_lat <- LM_area_res %>%
  ggplot() + 
  geom_point(aes(x = lat, y = SA_int, col = LME), alpha = 0.5, size = 1) +
  geom_line(aes(x = lat, y = .fitted, col = LME), size = 0.6) + 
  geom_ribbon(aes(x = lat, ymin = .lower, ymax = .upper, group = LME, fill = LME),
              alpha = 0.1) +
  scale_x_continuous("Latitude (°N)", breaks = seq(0, 90, 2)) + 
  scale_y_continuous(expression("S"[A]*" (dB re 1 m"^2*" nmi"^-2*")")) + 
  scale_colour_manual("Region", values = met.brewer("Johnson", n = 6,
                                                    direction = -1)) +
  scale_fill_manual("Region", values = met.brewer("Johnson", n = 6, 
                                                  direction = -1)) +
...
lm_SA_lat <- LM_area_res %>%
  mutate(LME = factor(case_when(LME == "BF" ~ "Beaufort Sea",
                                LME == "BB" ~ "Baffin Bay",
                                LME == "SV" ~ "Barents Sea"),
                      levels = c("Beaufort Sea", "Baffin Bay", "Barents Sea"))) %>%
  ggplot() + 
  geom_point(aes(x = lat, y = SA_int, col = LME), alpha = 0.5, size = 1) +
  geom_line(aes(x = lat, y = .fitted, col = LME), size = 0.6) + 
  geom_ribbon(aes(x = lat, ymin = .lower, ymax = .upper, group = LME, fill = LME),
...
GAMSAI_velo <- predSA_I %>%
  ...
  filter(var == "v") %>%
  ggplot() +
  geom_line(aes(x = v, y = fitted, col = LME), size = 0.6) +
  ...
  labs(x = expression("Mean velocity at 380 m (cm s"^-1*")"),
