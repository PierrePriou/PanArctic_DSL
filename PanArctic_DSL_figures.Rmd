...
load(paste0("data/remote_sensing/physics_grids_", "50", "km.RData"))
...
trawl_laea <- SpatialPointsDataFrame(
  ...
  st_join(., LME_sf, join = st_within) %>% # Append IHO region
  st_drop_geometry() %>%
...
SA_all_area <- SA_grid_laea %>%
  ...
  summarise(n = n(),
            ...
            NASC_mean2 = mean(NASC_mean, na.rm = T),
            NASC_sd2 = sd(NASC_mean, na.rm = T),
...
SA_grid_laea %>%
  ...
  summarise(n = n(),
            ...
            NASC_int_sd = sd(NASC_int, na.rm = T), 
            NASC_mean2 = mean(NASC_mean, na.rm = T),
            NASC_sd2 = median(NASC_mean, na.rm = T),
            CM_median = median(CM, na.rm = T),
            CM_mad = mad(CM, na.rm = T),
            CM_mean = mean(CM, na.rm = T),
            CM_sd = sd(CM, na.rm = T)) %>%
...
p_map_laea <- bathy_laea %>%
  ...
  scale_fill_cmocean("Depth (m)", name = "deep",
                     discrete = T, na.value = NA, alpha = 0.3) +
  new_scale_fill() +
  # Areas of interest
  # geom_polygon(data = IHO_WAO, aes(x = xc, y = yc, group = group),
  #              fill = NA, col = "red", lwd = 0.3, lty = 1) +
  # geom_shape(data = IHO_BF, aes(x = xc, y = yc, group = group),
  #            fill = NA, col = "purple", lwd = 0.3, lty = 1) +
  # geom_shape(data = IHO_CAA, aes(x = xc, y = yc, group = group),
  #            fill = NA, col = "orange", lwd = 0.3, lty = 1) +
  # geom_shape(data = IHO_BB, aes(x = xc, y = yc, group = group),
  #            fill = NA, col = "blue", lwd = 0.3, lty = 1) +
  # geom_shape(data = IHO_DS, aes(x = xc, y = yc, group = group),
  #            fill = NA, col = "green", lwd = 0.3, lty = 1) +
  # geom_shape(data = IHO_EAO, aes(x = xc, y = yc, group = group),
  #            fill = NA, col = "yellow", lwd = 0.3, lty = 1) +
  geom_polygon(data = LME, aes(x = xc, y = yc, group = group, fill = group),
               col = "red", alpha = 0.5, lwd = 0.3, lty = 1) +
  # Plot coastlines
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group),
               fill = "grey30") +
  # Arctic circle
  geom_path(data = arctic_circle_laea, aes(x = xc, y = yc), 
            col = "grey10", lty = 2, size = 0.3) +
  coord_fixed(xlim = c(-2450, 600), ylim = c(-1500, 1800), expand = F) +
  guides(fill = guide_legend(keywidth = 0.2, keyheight = 0.2,
                             default.unit = "in"), color = "none") +
  theme(legend.position = "right", 
        panel.border = element_rect(fill = NA),
        axis.text = element_blank(),
        axis.ticks = element_blank(), 
        axis.title = element_blank())
ggsave("plots/base_map_laeamap_stations.png",
       p_map_laea, height = 3, width = 6, dpi = 600, units = "in")
p_map_laea
```
...
map_DSL <- SA_grid_laea %>% 
  ...
  ggplot(aes(x = xc,  y = yc)) +
  geom_raster(data = bathy_laea, aes(x = xc, y = yc, fill = depth_d), alpha = 0.8) + # bathy
...
map_DSL <- SA_grid_laea %>% 
  ...
  # mutate(NASC_int_d = cut(NASC_int, c(0, 1, 5, 10, 50, 100, 500, 1000, 5000, 10000, Inf), right = F)) %>%
  ggplot(aes(x = xc,  y = yc)) +
  geom_raster(data = bathy_laea, aes(x = xc, y = yc, fill = depth_d), alpha = 0.8) + # bathy
  scale_fill_grey("Depth (m)", start = 0.9, end = 0.3) +
  guides(fill = "none") + 
  ...
  # scale_fill_viridis_d(expression("s"[A]*" ( m"^2*" nmi"^-2*")"),
  #                      option = "viridis",
  #                      labels = c("< 1", "1-4", "5-9", "10-49", "50-99",
...
pointrange_NASC <- SA_grid_laea %>% # Pointrange of SA per region
  ...
  geom_point(aes(y = 10*log10(NASC_mean)), shape = 18, size = 4.5, col = "black") +
...
```{r fig3-boxplot}
boxplot_NASC <- SA_grid_laea %>% # Pointrange of SA per region
  mutate(IHO_area = factor(
    case_when(
    IHO_area == "East Arctic Ocean" ~ "EAO",
    ...
    year = factor(year)) %>%
  ggplot(aes(x = IHO_area)) +
  geom_boxplot(aes(y = SA_int, fill = year), size = 0.4, outlier.size = 0.7,
               alpha = 0.8, position = position_dodge(preserve = "single")) +
  scale_fill_manual(values = c("#0086a8", "#f6c200", "#d04e00")) +
  scale_y_continuous(
    expression("S"[A]*" (dB re 1 m"^2*" nmi"^-2*")"),
    position = "right",
    sec.axis = 
  ...
  theme(axis.title.x = element_blank(),
        ...
        plot.margin = unit(c(0.1, 0.1, 0, 0), "in"))
# Save and display plot
ggsave("plots/Fig3_boxplot_NASC.png", boxplot_NASC, 
...
## Map/table 1. Abundance mesopelagic fish and map
...
trawl_station_df <- trawl %>%
  ...
  mutate(name = if_else(name == "The Northwestern Passages" & lon > -90, "Baffin Bay",
                if_else(name == "Arctic Ocean" & name_3 == "West Arctic Ocean", "West Arctic Ocean",
                if_else(name == "Arctic Ocean" & name_3 == "East Arctic Ocean", "East Arctic Ocean", name))),
         IHO_area = case_when(name == "East Arctic Ocean" ~ "EAO",
                              name == "West Arctic Ocean" ~ "WAO",
                              name == "Beaufort Sea" ~ "BF",
                              name == "The Northwestern Passages" ~ "CAA",
                              name == "Baffin Bay" ~ "BB",
                              name == "Davis Strait" ~ "DS"),
         fish_species2 = case_when(
           ...
           !fish_species %in% c("boreogadus_saida", "liparidae", "myctophidae", "sebastes_sp") ~ "Other")) %>%
  dplyr::select(-name, -fname, -id, -longitude, -latitude, -min_x, -min_y,
                -max_x, -max_y, -area.y, -mrgid, -bearings, -name_1, -mrgid_1, 
                -name_2,-mrgid_2, -name_3, -mrgid_3) %>%
  # Calculate number of individual for each species per station
  group_by(year, date, IHO_area, station, deployment, lat, lon, xc, yc, fish_species2) %>% 
  summarise(number_fish = sum(number_fish)) %>%
  ungroup() %>%
  # Calculate total number of fish per IHO region
  group_by(year, date, IHO_area, station, deployment, lat, lon, xc, yc) %>% 
  mutate(total_fish = sum(number_fish)) %>% 
  ungroup() %>%
  # Calculate relative abundance per station
  mutate(RA = (number_fish / total_fish) * 100) %>%
  dplyr::select(year, IHO_area, xc, yc, RA, fish_species2) %>% 
  pivot_wider(names_from = fish_species2, values_from = RA, values_fill = 0) %>%
  # Jitter coordinates to avoid overplotting
  mutate(row = seq(1, 18, 1),
         xc_jit = case_when(xc == xc[1] & yc == yc[1] ~ xc[1] - 50,
                            xc == xc[2] & yc == yc[2] ~ xc[2],
                            xc == xc[3] & yc == yc[3] ~ xc[3] - 50,
                            xc == xc[4] & yc == yc[4] ~ xc[4],
                            xc == xc[5] & yc == yc[5] ~ xc[5] + 25,
                            xc == xc[6] & yc == yc[6] ~ xc[6] + 25,
                            xc == xc[7] & yc == yc[7] ~ xc[7] + 25,
                            xc == xc[8] & yc == yc[8] ~ xc[8] - 50,
                            xc == xc[9] & yc == yc[9] ~ xc[9],
                            xc == xc[10] & yc == yc[10] ~ xc[10],
                            xc == xc[11] & yc == yc[11] ~ xc[11] + 140,
                            xc == xc[12] & yc == yc[12] ~ xc[12],
                            xc == xc[13] & yc == yc[13] ~ xc[13] - 140,
                            xc == xc[14] & yc == yc[14] ~ xc[14] + 25,
                            xc == xc[15] & yc == yc[15] ~ xc[15],
                            xc == xc[16] & yc == yc[16] ~ xc[16] + 75,
                            xc == xc[17] & yc == yc[17] ~ xc[17],
                            xc == xc[18] & yc == yc[18] ~ xc[18]),
         yc_jit = case_when(xc == xc[1] & yc == yc[1] ~ yc[1] - 75,
                            xc == xc[2] & yc == yc[2] ~ yc[2] + 25,
                            xc == xc[3] & yc == yc[3] ~ yc[3],
                            xc == xc[4] & yc == yc[4] ~ yc[4] - 100,
                            xc == xc[5] & yc == yc[5] ~ yc[5] - 75,
                            xc == xc[6] & yc == yc[6] ~ yc[6],
                            xc == xc[7] & yc == yc[7] ~ yc[7],
                            xc == xc[8] & yc == yc[8] ~ yc[8] - 100,
                            xc == xc[9] & yc == yc[9] ~ yc[9],
                            xc == xc[10] & yc == yc[10] ~ yc[10] + 75,
                            xc == xc[11] & yc == yc[11] ~ yc[11],
                            xc == xc[12] & yc == yc[12] ~ yc[12],
                            xc == xc[13] & yc == yc[13] ~ yc[13],
                            xc == xc[14] & yc == yc[14] ~ yc[14] + 50,
...
map_RA <- trawl_station_df %>%
  ...
  # geom_text(aes(x = xc_jit, y = yc_jit, label = row), size = 2, col = "green") +
...
## Figure 4. Linear regression latitude
...
lm_SA_lat <- LM_area_res %>%
  ggplot() + 
  geom_point(aes(x = lat, y = SA_int, col = IHO), alpha = 0.5, size = 1) +
  geom_line(aes(x = lat, y = .fitted, col = IHO), size = 0.6) + 
  geom_ribbon(aes(x = lat, ymin = .lower, ymax = .upper, group = IHO, fill = IHO), alpha = 0.1) +
  scale_x_continuous("Latitude (°N)", breaks = seq(0, 90, 2)) + 
  scale_y_continuous(expression("S"[A]*" (dB re 1 m"^2*" nmi"^-2*")")) + 
  scale_colour_manual("Region", values = met.brewer("Johnson", n = 6, direction = -1)) +
  scale_fill_manual("Region", values = met.brewer("Johnson", n = 6, direction = -1)) +
...
ggsave("plots/Figure_LM_SA_lat.png", lm_SA_lat, 
       height = 2.25, width = 3.55, dpi = 600, units = "in")
lm_SA_lat
```
...
lm_SA_lat <- LM_area_res %>%
  ...
  theme(legend.position = "top", 
        ...
        axis.line = element_line())
# Combine plots
p_map_NASC_LM <- plot_grid(map_DSL, lm_SA_lat,
                           ncol = 1, align = "v", axis = "lr",
                           labels = "auto", label_size = 9, label_y = c(0.9, 1)) 
# Save and display plot
...
# GAM chlorophyll
GAMSAI_chl <- predSA_I %>%
  mutate(lty = factor(case_when(IHO %in% c("WAO_BF", "EAO") ~ 1,
                                !IHO %in% c("WAO_BF", "EAO") ~ 2)),
         IHO = as.character(IHO),
         IHO = factor(
           case_when(IHO == "WAO_BF" ~ "West Arctic Ocean\n& Beaufort Sea", 
                     IHO == "CAA" ~ "Can. Arctic Archipelago", 
                     IHO == "BB" ~ "Baffin Bay",
                     IHO == "DS" ~ "Davis Strait", 
                     IHO == "EAO" ~ "East Arctic Ocean"),
           levels = c("West Arctic Ocean\n& Beaufort Sea",
                      "Can. Arctic Archipelago", "Baffin Bay", "Davis Strait",
                      "East Arctic Ocean"))) %>%
  filter(var == "chl") %>%
  ggplot() +
  geom_line(aes(x = chl, y = SA_fit, col = IHO, linetype = lty), size = 0.6) +
  geom_ribbon(aes(x = chl, ymin = SA_lwr_ci, ymax = SA_upr_ci, fill = IHO),
              alpha = 0.05) +
  scale_colour_manual(values = met.brewer("Johnson", n = 6, direction = -1)) +
  scale_fill_manual(values = met.brewer("Johnson", n = 6, direction = -1)) +
  coord_cartesian(xlim = c(0, 1.7), ylim = c(0, 47.5)) +
  scale_x_continuous(breaks = seq(0, 10, 0.5)) +
  labs(x = expression("Mean chlorophyll (mg m"^-3*")"),
...
GAMSAI_velo <- predSA_I %>%
  ...
  filter(var == "v") %>%
  ggplot() +
  geom_line(aes(x = v, y = SA_fit, col = IHO, linetype = lty), size = 0.6) +
  geom_ribbon(aes(x = v, ymin = SA_lwr_ci, ymax = SA_upr_ci, fill = IHO), 
              alpha = 0.05) +
  scale_colour_manual(values = met.brewer("Johnson", n = 6, direction = -1)) +
  scale_fill_manual(values = met.brewer("Johnson", n = 6, direction = -1)) +
  coord_cartesian(xlim = c(0, 4.1), ylim = c(0, 47.5)) +
...
# Legend for plots
legend <- get_legend(GAMSAI_chl + 
  guides(colour = guide_legend(nrow = 1, byrow = T)) +
  theme(legend.position = "top"))

# Combine plots
HGAM <- plot_grid(legend, 
                  plot_grid(GAMSAI_chl, GAMSAI_velo, ncol = 2,
                            labels = "auto", label_size = 9,
                            align = "hv", axis = "tblr"),
                  ncol = 1, rel_heights = c(0.2, 1))
# Save and display plot
ggsave("plots/Figure_HGAM_SAI3.png", HGAM, 
       height = 2.5, width = 6.5, dpi = 600, units = "in")
HGAM
```
...
## Figure SX Map chlorophyll a and current velocity

```{r map-chla-velo}
# Map chlorophyll a
map_chla <- chl_grid_laea_150 %>%
  ggplot() +
  # Bathymetry
  geom_raster(data = bathy_laea, aes(x = xc, y = yc, fill = depth_d),
              alpha = 0.8) +
  ...
  scale_fill_cmocean(expression("Chl a\n(mg m"^-3*")"),
                       name = "tempo", limits = c(0, 2.5),
                       na.value = "#151D44FF") +
  # Coastlines
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group), 
               fill = "grey25") + 
  coord_fixed(xlim = c(-2450, 650), ylim = c(-1500, 1850), expand = F) +
  # Coords 
  geom_tile(data = SA_grid_laea, aes(x = xc, y = yc), fill = NA,
...
# Combine plots
map_env <- plot_grid(map_chla, map_velo, ncol = 1, align = "hv", axis = "tblr",
                     labels = "auto", label_size = 9)
# Save and display plot
ggsave("plots/Figure_map_chl_velo.png", map_env, 
       height = 5, width = 6.5, dpi = 600, units = "in")
```
