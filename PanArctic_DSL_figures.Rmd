...
pointrange_NASC <- SA_grid_laea %>% # Pointrange of SA per region
  ...
  geom_pointrange(aes(y = 10*log10(NASC_median), ymin = 10*log10(NASC_q025), ymax = 10*log10(NASC_q975)),
...
trawl_station_df <- trawl %>%
  ...
  mutate(row = seq(1, 18, 1),
         xc_jit = case_when(xc == xc[1] & yc == yc[1] ~ xc[1] - 50,
                            ...
                            xc == xc[11] & yc == yc[11] ~ xc[11] + 150,
                            xc == xc[12] & yc == yc[12] ~ xc[12],
                            xc == xc[13] & yc == yc[13] ~ xc[13] - 150,
         ...
         yc_jit = case_when(xc == xc[1] & yc == yc[1] ~ yc[1] - 75,
                            ...
                            xc == xc[11] & yc == yc[11] ~ yc[11] + 50,
                            xc == xc[12] & yc == yc[12] ~ yc[12],
                            xc == xc[13] & yc == yc[13] ~ yc[13] - 50,
                            xc == xc[14] & yc == yc[14] ~ yc[14] + 50,
                            xc == xc[15] & yc == yc[15] ~ yc[15],
                            xc == xc[16] & yc == yc[16] ~ yc[16] - 100,
...
map_RA <-
  ...
  geom_raster(data = bathy_laea,
              aes(x = xc, y = yc, fill = depth_d), alpha = 0.8) + 
  scale_fill_grey("Depth (m)", start = 0.9, end = 0.3) +
  guides(fill = "none") +
  new_scale_fill() +
  # Coastlines
  ...
  # geom_text(aes(x = xc_jit, y = yc_jit, label = row), size = 8, col = "green") +
