...
load(paste0("data/remote_sensing/chl_grids_", "25", "km.RData"))
...
trawl_laea <- SpatialPointsDataFrame(
  ...
  st_drop_geometry() %>%
  mutate(name = if_else(name == "The Northwestern Passages" & coords.x2 < 0, "Baffin Bay",
  ...
  dplyr::select(year, IHO_area, lat, lon, xc, yc)
...
trawl_station_df <- trawl %>%
  ...
  mutate(name = if_else(name == "The Northwestern Passages" & lon > -90, "Baffin Bay",
         ...
         fish_species2 = case_when(fish_species == "boreogadus_saida" ~ "Arctic cod",
...
GAMSAI_velo <- predSA_I %>%
  mutate(lty = factor(case_when(IHO %in% c("WAO_BF", "BB", "EAO") ~ 1,
                                !IHO %in% c("WAO_BF", "BB", "EAO") ~ 2))) %>%
  filter(var == "v") %>%
  ggplot() +
...
GAMSAI_chl <- predSA_I %>%
  ...
  scale_colour_manual(values = met.brewer("Johnson", n = 6, direction = -1)) +
  scale_fill_manual(values = met.brewer("Johnson", n = 6, direction = -1)) +
  coord_cartesian(xlim = c(0, 2.1), ylim = c(0, 40), expand = T) +
...
HGAM <- plot_grid(legend, 
                  plot_grid(GAMSAI_chl, GAMSAI_velo, ncol = 2, labels = "auto", label_size = 9),
...
map_chla <- chl_grid_laea_50 %>%
  ...
  # new_scale_fill() + 
  # Chlorophyll a
  geom_tile(aes(x = xc, y = yc, fill = chl)) + 
  # scale_fill_viridis_c(expression("Chl a (mg m"^-3*")"),
  #                      option = "viridis", limits = c(0, 2.5), 
  #                      na.value = "#FDE725FF") +
  scale_fill_cmocean(expression("Chl a (mg m"^-3*")"),
                       name = "algae", limits = c(0, 2.5), 
                       na.value = "#122414FF") +
  # Coastlines
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group), 
               fill = "grey25") + 
  coord_fixed(xlim = c(-2450, 1000), ylim = c(-1500, 1850), expand = F) +
  # Coords 
...
map_velo <- phy_grid_laea_50 %>%
  ...
  scale_fill_cmocean(expression("Velocity (cm s"^-1*")"),
...
map_env <- plot_grid(map_chla, map_velo, ncol = 1, align = "hv", axis = "tblr",
                     labels = "auto", label_size = 9)
# Save and display plot
ggsave("plots/Figure_map_chla.png", map_chla, 
       height = 2.5, width = 6.5, dpi = 600, units = "in")
