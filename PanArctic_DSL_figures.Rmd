...
# col_pal <- c("#5BBCD6", "#F2AD00", "#FF0000", "black")
col_pal <- c("#1170aa", "#fc7d0b", "#76B7B2", "black") # Custom colour palette
...
Sv_prof_median_year <- Sv_grid_laea %>%
  left_join(., empty_DSL, by = c("year", "xc", "yc")) %>% # Add DSL presence
  filter(empty_clean == F) %>%
  mutate(IHO_area = factor(case_when(IHO_area == "East Arctic Ocean" ~ "East Arctic\nOcean",
                                     IHO_area == "West Arctic Ocean" ~ "West Arctic\nOcean",
  ...
  group_by(depth_round, IHO_area, year) %>%
  summarise(sv_lin50 = median(sv_lin_median)) %>% 
  ungroup() %>%
  mutate(Sv_median = 10 * log10(sv_lin50),
...
Sv_prof_median <- Sv_grid_laea %>%
  ...
  mutate(Sv_median = 10 * log10(sv_lin50),
         year = "Median") %>%
  bind_rows(., Sv_prof_median_year) %>%
  mutate(year = factor(year, levels = c("2015", "2016", "2017", "Median")))
...
p_Sv_prof_lines <- Sv_prof_median %>%
  ggplot() +
  # geom_line(data = Sv_prof, aes(x = depth_round, y = Sv_median, group = ID), col = "grey85", size = 0.5) +
  # geom_vline(xintercept = 200, lty = 3) + # Mesopelagic zone
  geom_line(aes(x = depth_round, y = Sv_median, col = year), size = 0.7) + # Sv profiles per year and median
  ...
  geom_rect(aes(xmin = 0, xmax = 200, ymin = -Inf, ymax = Inf), col = NA, fill = "grey90", alpha = 0.05) + # Mesopelagic zone
  scale_x_reverse("Depth (m)", breaks = seq(0, 1000, 200)) + 
...
ggsave("plots/Fig2_Sv_profiles_lines.png", p_Sv_prof_lines, height = 3.5, width = 8, dpi = 600, units = "in")
p_Sv_prof_lines
```
