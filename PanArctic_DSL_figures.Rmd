...
SA_all_area <- SA_grid_laea %>%
  ...
  summarise(n = n(),
            NASC_median = median(NASC_int),
            NASC_mad = mad(NASC_int), 
            CM_median = median(CM),
            CM_mad = mad(CM)) %>%
  ungroup() %>%
...
SA_grid_laea %>%
  ...
  summarise(n = n(),
            NASC_median = round(median(NASC_int), 1),
            NASC_mad = round(mad(NASC_int), 1),
...
p_boxplot_SA <- SA_grid_laea %>% # Boxplot of SA per region
  mutate(IHO_area = factor(case_when(IHO_area == "East Arctic Ocean" ~ "East Arctic\nOcean",
                                     IHO_area == "West Arctic Ocean" ~ "West Arctic\nOcean",
                                     IHO_area == "Beaufort Sea" ~ "Beaufort Sea",
                                     IHO_area == "The Northwestern Passages" ~ "Can. Arctic\nArchipelago",
...
map_DSL <- SA_grid_laea %>%  # Map of DSL detections
  ...
  mutate(DSL = factor(if_else(empty_clean == T, "Absent", "Present"), levels = c("Present", "Absent"))) %>%
  arrange(DSL) %>%
  ggplot(aes(x = xc,  y = yc)) +
  geom_raster(data = bathy_laea, aes(x = xc, y = yc, fill = depth_d)) + # bathy
  scale_fill_cmocean("Depth (m)", name = "deep", discrete = T, na.value = NA, alpha = 0.4) +
  ...
  scale_color_viridis_d(option = "plasma", labels = c("< 1", "1-10", "11-100", "101-1000", "1001-10000", "> 10000")) +
  facet_wrap(~ year) +
  coord_fixed(xlim = c(-2450, 600), ylim = c(-1500, 1800), expand = F) +
  # coord_fixed(xlim = c(-2600, 1100), ylim = c(-1600, 1900), expand = F) +
  guides(fill = "none",
         colour = guide_legend(override.aes = list(size = 2), nrow = 1)) + # Increase symbol size in legend
  theme(legend.position = "top",
        legend.title = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank(),
...
pointrange_NASC <- SA_grid_laea %>% # Pointrange of SA per region
  ...
  scale_colour_manual(values = met.brewer("Johnson", n = 6, direction = -1)) +
  scale_y_continuous(expression("Median s"[A]*" (m"^2*" nmi"^-2*")")) +
  theme(panel.border = element_blank(),
...
p_map_pointrange_NASC <- plot_grid(map_DSL, pointrange_NASC, ncol = 1, align = "v", axis = "lr") 
ggsave("plots/Fig3_map_pointrange_NASC.png", p_map_pointrange_NASC, height = 5, width = 6, dpi = 600, units = "in")
