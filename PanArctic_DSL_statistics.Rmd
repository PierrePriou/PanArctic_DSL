...
load("data/remote_sensing/physics_grids_266_318.RData") # Modelled physics data 
...
phy_laea2 <- phy_grid_laea_266_318 %>% # Remote sensing: physics reanalysis
  dplyr::select(year, area, xc, yc, cell_res, depth, thetao, velocity, so)
phy_laea3 <- phy_grid_laea %>% 
  bind_rows(phy_grid_laea_266_318) %>% # Remote sensing: physics reanalysis
  dplyr::select(year, area, xc, yc, cell_res, depth, velocity) %>%
  filter(depth %in% c(222, 266, 318, 380)) %>%
  group_by(year, area, xc, yc, cell_res) %>%
  summarise(velocity = mean(velocity)) %>%
  mutate(depth = round(mean(c(222, 266, 318, 380))))
...
stat_laea <- SA_laea %>%
  ...
  left_join(., phy_laea2, by = c("year", "xc", "yc", "area", "cell_res")) %>%
  # left_join(., phy_laea3, by = c("year", "xc", "yc", "area", "cell_res")) %>%
...
```{r normality-check, fig.height=8}
# Normality: QQ plot
...
```{r GAM-prep}
SA_df <- stat_laea %>%
  dplyr::select(year, xc, yc, IHO_area, NASC_int, SA_int, velocity, openwater_duration, mean_ice_conc, chl) %>%
  group_by(IHO_area) %>%
...
```{r GAMI-pred}
ilink_I <- family(GAM_S)$linkinv # Get link function
pred_I <- predict(GAM_S, SA_new, type = "link", se.fit = TRUE) %>% # Predict data
  bind_cols(., SA_new) %>%
...
```{r GAMI-ggplot, fig.height=8, fig.width=8}
plot_grid(
  pred_I %>%
    filter(var == "v") %>%
    ggplot() +
