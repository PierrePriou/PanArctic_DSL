...
# Map projections
cell_res <- 200 # Cell resolution in km
...
lm_gaussian <- lm(NASC_anomaly ~ cons_temp + mean_ice_conc + openwater_duration, 
                  data = SA_laea)
summary(lm_gaussian)
autoplot(lm_gaussian)
```
```{r}
test <- stat_laea %>% # Calculate normalized backscatter anomalies
  group_by(area, year) %>%
  mutate(mean_NASC_area_year = mean(NASC_int),
         sd_NASC_area_year = sd(NASC_int)) %>%
  ungroup() %>%
  mutate(year = as.factor(year),
         NASC_ano = (NASC_int - mean_NASC_area_year) / sd_NASC_area_year,
```
