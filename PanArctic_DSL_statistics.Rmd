...
```{r map-velocity, fig.height=3.5, fig.width=6}
# Velocity
phy_grid_laea %>% 
  filter(depth == 318) %>%
  ggplot(aes(x = xc,  y = yc)) +
...
```{r GAMGI-residuals-covariates, message=FALSE}
...
```{r GAMGS-acf-pacf}
par(mfrow = c(1, 2))
acf(resid(GAM_GS), lag.max = 36, main = "ACF")
pacf(resid(GAM_GS), lag.max = 36, main = "pACF")
```

```{r GAMGI-acf-pacf}
par(mfrow = c(1, 2))
acf(resid(GAM_GI), lag.max = 36, main = "ACF")
pacf(resid(GAM_GI), lag.max = 36, main = "pACF")
```
...
```{r GAMGS2-residuals-covariates, message=FALSE}
appraise(GAM_GS2)
...
```{r GAMGI2-residuals-covariates, message=FALSE}
appraise(GAM_GI2)
...
```{r GAMGS-pretty-ggplot, fig.height=8, fig.width=7}
GAMGS_velo <- pred_GS %>%
  filter(var == "v") %>%
  ggplot() +
