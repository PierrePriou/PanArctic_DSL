...
for (i in seq(2015, 2017, 1)) { # Data gridding
  ...
  SA_tmp_laea <- SpatialPointsDataFrame(
    ...
    rasterize(., arctic_laea, fun = median, na.rm = T) %>% # Rasterize data in latlon
    ...
    mutate(year = i, 
           area = factor(case_when(lon > -155 & lon <= -95 & lat > 65 & lat <= 82 ~ "BF_CAA",
                                   ...
                                   lon >= -25 & lon <= 145 & lat > 77 & lat <= 90 ~ "SV"),
                         levels = c("BF_CAA", "BB", "SV"))) %>%
    dplyr::select(year, area, lat, lon, xc, yc, NASC_int, NASC_int_clean, NASC_mean, CM) 
...
SA_grid_laea <- SA_grid_laea %>% # Calculate normalized backscatter anomalies
  ...
  dplyr::select(year, xc, yc, lon, lat, IHO_area, area, NASC_int, NASC_int_clean, NASC_mean, SA_int, SA_int_clean, CM, empty, empty_clean, cell_res)
```
