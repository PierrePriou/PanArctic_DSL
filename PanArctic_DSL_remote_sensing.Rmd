...
```{r}
# Read NetCDF
test <- tidync("data/remote_sensing/physics/20150115_0m_phy_CMEMS.nc") %>%
...
proj_original <- "+proj=stere +lat_0=90 +lat_ts=90 +lon_0=-45 +x_0=0 +y_0=0 +R=6378273 +units=m"


# Project data
test_latlon <- SpatialPointsDataFrame(SpatialPoints(cbind(test$x, test$y), proj4string = CRS(proj_original)),
                                      data.frame(thetao = test$thetao,
                                                 vxo = test$vxo,
                                                 vyo = test$vyo,
                                                 so = test$so,
                                                 depth = test$depth,
...
bbox(test_latlon) # Latitude is off, longitude seems fine

plot(test_latlon, axes = T)  
```