...
cell_res <- 50 # Cell resolution in km
arctic_laea <- raster(extent(-2700, 2700, -2700, 2700),
                      crs = "EPSG:6931") # Seaice projection
projection(arctic_laea) <- gsub("units=m", "units=km",
                                projection(arctic_laea)) 
res(arctic_laea) <- c(cell_res, cell_res) 

# List files per yeaR
file_list <- list.files("data/remote_sensing/ocean_colour/nc_OCEANCOLOUR_ARC_CHL_L4_REP_OBSERVATIONS_009_088",
                        pattern = ".nc", full.names = T)

for (i in file_list) { # Loop through each file
  date_file <- str_remove(i, pattern = "data/remote_sensing/ocean_colour/nc_OCEANCOLOUR_ARC_CHL_L4_REP_OBSERVATIONS_009_088/") %>%
  ...
  if (nrow(chl_tmp > 0)){
    ...
    chl_tmp_laea <- SpatialPointsDataFrame(
      ...
      rasterize(., arctic_laea, fun = mean, na.rm = T) %>% # Rasterize on laea
      dropLayer(1) %>% # Remove ID layer
      rasterToPoints() %>% # Convert raster to data frame
      as.data.frame() %>%
      rename(xc = x, yc = y) %>% # Rename variables
...
Plot chlorophyll in 2015, 2016, 2017.
...
chl_grid_laea %>% 
  ...
  scale_fill_cmocean("Chl", name = "algae", limits = c(0, 4)) +
