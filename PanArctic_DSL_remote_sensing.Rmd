...
# Date range of interest
dates <- seq(ymd("2017-06-10"), ymd("2017-12-31"), 1)
# Download data
for (d in dates) {
  # Format years, month, and day to be correctly read in the request
  year = as.character(year(as.Date(d, origin = "1970-01-01")))
...
Plot data to see if download worked.

```{r plot-seaice-latlon-cdr, fig.align='center', fig.height=3.25, fig.width=6, message=FALSE, warning=FALSE, dpi=600}
...
```{r plot-seaice-latlon-icdr, fig.align='center', fig.height=3.25, fig.width=6, message=FALSE, warning=FALSE, dpi=600}
...
I calculate the day of ice break-up, sea ice and open-water duration, and mean sea ice concentration for each cell. I calculate open-water duration from one sea ice minimum extent to the next. I retrieved the dates of the sea ice minimum extent from [NSIDC](http://nsidc.org/arcticseaicenews/2021/09/arctic-sea-ice-at-highest-minimum-since-2014/). We define a cell being ice-covered when the mean sea ice concentration is above 15 % [(as per NSIDC Sea Ice Index)](https://nsidc.org/cryosphere/glossary/term/ice-extent).
...
seaice_raw <- list.files("data/remote_sensing/sea_ice", pattern = "*laea_ice_conc.csv", full.names = T) %>% # List files in folder
  ...
  dplyr::select(- time)
# seaice_2deg_season <- seaice_raw %>% # Calculate metrics for each season
...
#             openwater_duration = total_day - seaice_duration,
#             mean_ice_conc = round(mean(ice_conc, na.rm = T), 2))
save(seaice_raw, file = "data/remote_sensing/remote_sensing_seaice_raw.RData") # Save data
```

Plot data to see if calculations worked.

```{r map-openwater-duration, fig.align='center', fig.height=6, fig.width=6, warning=FALSE, dpi=600}
seaice_2deg_year %>%
...
```{r map-mean-ice-concentration, fig.align='center', fig.height=6, fig.width=6, warning=FALSE, dpi=600}
seaice_2deg_year %>%
