...
SA_grid_laea <- SA_grid_laea %>% # Calculate normalized backscatter anomalies
  ...
  mutate(name = if_else(is.na(name) == T & yc < 0, "Baffin Bay",
         ...
         IHO_area_large = factor(case_when(name == "Beaufort Sea" ~ "BF_WAO",
                                           name == "West Arctic Ocean" ~ "BF_WAO",
                                           name == "Baffin Bay" ~ "BB_DS",
                                           name == "Davis Strait" ~ "BB_DS",
                                           name == "The Northwestern Passages" ~ "CAA",
                                           name == "East Arctic Ocean" ~ "EAO"),
                                 levels = c("BF_WAO", "CAA", "BB_DS", "EAO"))) %>%
           rename(IHO_area = name, area = area.x) %>%
  dplyr::select(year, xc, yc, lon, lat, IHO_area, IHO_area_large, area, NASC_int, CM, mean_NASC_area_year, sd_NASC_area_year,
...
SA_grid_laea %>%
  ...
  geom_point(aes(x = xc, y = yc, col = IHO_area_large)) +
  coord_fixed(xlim = c(-2600, 1100), ylim = c(-1800, 1900), expand = F)
```

![](PanArctic_DSL_acoustics_files/figure-gfm/plot-areas-1.png)<!-- -->
...
## 3D S<sub>V</sub> profiles - EPSG:6931 - EASE-Grid 2.0 North (Lambert’s equal-area, azimuthal)
...
coord_equivalences <- SA_grid_laea %>% # Calculate coord equivalences between 4326 and 6931
  group_by(xc, yc, area, IHO_area, IHO_area_large) %>%
  summarise()
...
Sv_grid_laea <- Sv_grid_laea %>% # Tidy data
  ...
  dplyr::select(year, xc, yc, lon, lat, IHO_area, IHO_area_large, area, depth, cell_res,
...
![](PanArctic_DSL_acoustics_files/figure-gfm/3D-EPSG-6931-Sv-profiles-1.png)<!-- -->
