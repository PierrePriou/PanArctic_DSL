...
2°lon x 1°lat, and the EASE-Grid 2.0 North (EPSG:6931) with grid cells
of 100 km x 100 km—which is the default grid for sea-ice data. For each
cell I calculated the mean integrated nautical area scattering
...
for (i in seq(2015, 2017, 1)) { # Data gridding
  ...
  SA_tmp_latlon <- SpatialPointsDataFrame(SpatialPoints(cbind(SA_tmp$lon, SA_tmp$lat), 
    ...
    group_by(year, area) %>%
    mutate(NASC_anomaly = (NASC_int - mean(NASC_int)) / sd(NASC_int),  # Calculate normalized backscatter anomaly
...
plot_grid(SA_grid_laea %>% # Map integrated normalized backscatter anomaly
            ...
            guides(fill = guide_legend(keywidth = 0.05,
                   ...
                   color = "none") +
            theme(legend.position = "top"),
          SA_grid_laea %>% # Map absolute salinity
            ggplot() +
            geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group), fill = "grey80") +
