...
```{r fig1-map-station-original, fig.align='center', fig.height=3.25, fig.width=6, dpi=600}
MVBS %>%
  mutate(lat = round(lat, 1),
         lon = round(lon, 1)) %>%
  group_by(lat,lon) %>%
...
```{r fig1-map-station-laea, fig.align='center', fig.height=3.25, fig.width=6, dpi=600}
...
MVBS_laea %>%
  ...
  scale_fill_cmocean("Depth (m)", name = "deep", discrete = T, na.value = NA, alpha = 0.7) +
  # Plot coastlines
...
```{r fig2-Sv-profile-area-year, fig.align='center', dpi=100, fig.width=6.5, fig.height=3, dpi=600}
...
```{r fig3-map-sa-anomalies-laea, fig.align='center', fig.height=3.25, fig.width=6, dpi=600}
# Map integrated normalized backscatter anomaly
SA_grid_laea %>% 
  ggplot() +
  geom_polygon(data = coast_10m_laea, aes(x = xc, y = yc, group = group), fill = "grey80") +
  geom_tile(aes(x = xc, y = yc, fill = NASC_anomaly_d), color = "grey30", alpha = 0.8, size = 0.15) +
  scale_fill_manual("EPSG:6931 - Normalized sA anomaly",
                    values = rev(brewer.pal(n = 5, name = "BrBG"))) +
...
I selected acoustic data within 1 hour of a CTD cast and calculated the mean mesopelagic NASC (between 200 and 1000 m depth) and mean mesopelagic temperature and salinity.

```{r MVBS-CTD-data-manip, include=T}
# Prepare CTD data
CTD <- CTD_2015_2019 %>% 
...
MVBS_CTD <- MVBS %>%
  ...
  group_by(date_CTD_cast, area, cast, lat_CTD, lon_CTD, CT, SA) %>%
  summarise(NASC_int = mean(NASC_int)) %>% # Calculate mean mesopelagic NASC for each CTD cast
  ungroup() %>%
  mutate(SA_int = 10 * log10(NASC_int))
```

```{r SA-temp-sal, fig.align='center', fig.width=6, fig.height=3, dpi=600}
